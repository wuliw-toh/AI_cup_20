...
class MyStrategy:
    ...
    def get_action(self, player_view, debug_interface):
        ...
        for ent in n_m:
            if ent.entity_type == EntityType.BUILDER_UNIT:
                self.squad.add_unit(ent)

        #print(self.squad.population, self.squad.entities)
        #Стратегия быстрого старта
        if self.squad.population < 5:
            ...
            if player_view.current_tick < 70:
                self.squad.go_to_point()
            else:
                self.squad.atacka()

            result = self.squad.result
